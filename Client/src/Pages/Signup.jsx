...
const Signup = ({ toggleForm }) => {
  ...
  const handleChooseResume = (event) => {
    const file = event.target.files[0];
    setFormValues({...formValues,resume: file});
  };

  const handleRemoveResume=(event)=>{
    setFormValues({...formValues,resume: null});
  ...
  const handleUploadImage = () => {
    ...
    // This function will be triggered when the "Upload Image" button is clicked
    console.log("Image upload functionality goes here");
  };

  const handleUploadResume = () => {
    // Logic for uploading resume
    // This function should handle the resume upload process
  };
  ...
  return (
    ...
    {/* Phone number feild starts */}
        ...
        <div className="flex items-center">
          ...
          <button
            ...
            className={`ml-4 rounded-lg bg-indigo-600 px-3 py-2 text-sm font-medium text-white ${otpButtonClicked ? 'opacity-50 cursor-not-allowed' : ''}`}
            disabled={otpButtonClicked}
          >
            {otpButtonClicked ? 'Sent' : 'Send OTP'}
          </button>
        </div>
        ...
        {/* Otp input feild ends  */}

    {/* Phone Number feild ends */}
        ...
        <div className="mt-1 flex items-center space-x-4">
            ...
            <span className="w-full h-12 bg-gray-200 rounded-md flex items-center justify-center text-center text-gray-400 px-20">
            ...
            <button
                ...
                className="bg-red-500 hover:bg-red-600 text-white px-1 py-2 rounded-md text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
            >
                Remove
            </button>
            )}
    ...
    {/* Resume feild starts */}
        ...
        <label className="relative overflow-hidden">
            ...
            name="resume"
            className="sr-only"
            onChange={handleChooseResume}
            />
        </label>

        <button
            onClick={handleUploadResume}
            className="bg-indigo-600 hover:bg-indigo-700 text-white px-1 py-2 rounded-md text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
        >
            Upload Resume
        </button>
        {formValues.resume && (
