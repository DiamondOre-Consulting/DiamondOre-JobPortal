...
const EachCandidate = () => {
    ...
    const fetchCandidate = async () => {
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/admin-confi/all-candidates/${id}`,
          {
            headers: {
  ...
  useEffect(() => {
    const fetchCandidateAppliedJobs = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_BASE_URL}/admin-confi/all-applied-jobs/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
