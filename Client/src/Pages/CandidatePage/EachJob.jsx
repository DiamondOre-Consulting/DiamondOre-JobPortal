    ...
    const fetchEachJob = async () => {
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/candidates/all-jobs/${id}`,
          {
            headers: {
  ...
  const applyJob = async () => {
    ...
    try {
      const apply = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/candidates/apply-job/${id}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
              ...
              {statusCheck?.status?.Applied ? (
                ...
                <>
                  ...
                  {!applyRes && (
                    <button
                      onClick={applyJob}
                      className="inline-block rounded-lg bg-white px-8 py-3 text-center text-sm font-semibold text-gray-800 outline-none ring-indigo-300 transition duration-100 hover:bg-gray-100 focus-visible:ring active:bg-gray-200 md:text-base" disabled={loader}
                    >
                      {loader ? (
