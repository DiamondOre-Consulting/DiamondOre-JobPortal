    ...
    const fetchAllDirectJobs = async () => {
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/candidates/all-direct-jobs`,
          {
            headers: {
              ...
              .map((directJob) => (
                <div className="flex flex-col justify-between h-64 overflow-hidden rounded-lg bg-white shadow-lg shadow-2xl-gray-200 p-4 shadow-lg hover:shadow-2xl">
                  ...
                  <p className="text-sm text-gray-600 font-semibold">
                    Industry -{" "}
                    <span className="text-blue-950">{directJob?.Industry}</span>
                  </p>
                  <p className="text-sm text-gray-600 font-semibold">
                    Channel -{" "}
                    <span className="text-blue-950">{directJob?.Channel}</span>
                  </p>
                  <p className="text-sm text-gray-600 font-semibold">
                    Min. Experience -{" "}
                    <span className="text-blue-950">
                      {directJob?.MinExperience} Year(s)
                    </span>
                  </p>

                  {directJob?.appliedApplicants == decodedToken?.userId ? (
