...
const CandidateAllBancaJobsCards = () => {
  ...
  const pageCount = Math.ceil(bancaJobs.length / jobsPerPage);
    ...
    const fetchAllbancaJobs = async () => {
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/candidates/all-banca-jobs`,
          {
            headers: {
        ...
        if (response.status == 200) {
          ...
          const filteredJobs = all.filter((job) => job.jobStatus === "Active");
          setBancaJobs(response?.data);
          setLoading(false);
        }
      } catch (error) {
  ...
  console.log("banca jobs", bancaJobs);
  return (
              ...
              .map((bancaJob) => (
                <div className="flex flex-col justify-between h-64 overflow-hidden rounded-lg bg-white shadow-lg shadow-2xl-gray-200 p-4 shadow-lg hover:shadow-2xl">
                  ...
                  <p className="text-sm text-gray-600 font-semibold">
                    Industry -{" "}
                    <span className="text-blue-950">{bancaJob?.Industry}</span>
                  </p>
                  <p className="text-sm text-gray-600 font-semibold">
                    Channel -{" "}
                    <span className="text-blue-950">{bancaJob?.Channel}</span>
                  </p>
                  <p className="text-sm text-gray-600 font-semibold">
                    Min. Experience -{" "}
                    <span className="text-blue-950">
                      {bancaJob?.MinExperience} Year(s)
                    </span>
                  </p>

                  {bancaJob?.appliedApplicants == decodedToken?.userId ? (
