      ...
      const fetchAllagencyJobs = async () => {
        try {
          ...
          const response = await axios.get(
            "http://localhost:5000/api/candidates/all-agency-jobs",
            {
                headers: {
      ...
      <div class="mx-auto max-w-screen-2xl px-4 md:px-8">
        ...
        {agencyJobs.map((agencyJob) => (
              ...
              >
                ...
                <p className="text-xl text-gray-600 font-semibold">Channel - <span className="text-blue-950">{agencyJob?.Channel}</span></p>
                <p className="text-xl text-gray-600 font-semibold">
                  Min. Experience - <span className="text-blue-950">{agencyJob?.MinExperience} Year(s)</span>
                </p>
                <p className="text-xl text-gray-600 font-semibold">
                  Total Openings - <span className="text-blue-950">{agencyJob?.Vacancies}</span>
                </p>
                ...
                <Link to={`/all-jobs/${agencyJob?._id}`} class="cursor-pointer w-full flex-col rounded-lg bg-white p-4 text-center hover:bg-blue-950 text-gray-800 hover:text-gray-200">
