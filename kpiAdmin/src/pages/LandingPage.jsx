...
const LandingPage = () => {
  ...
  const validateToken = async () => {
    setLoading(true)
    try {
      const res = await fetch(`${import.meta.env.VITE_BASE_URL}/admin-confi/validate-token-for-kpi-admin`, {
        method: "GET",
        headers: {
      ...
      if (res.ok && (data.user?.role === "kpiAdmin"||data.user?.role === "superAdmin")) {
        navigate("/kpi");
      }
    } catch (err) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_BASE_URL}/admin-confi/login-kpi-admin`,
        {
          email,
          password
        });
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-50">
      <div className="max-w-screen-xl sm:max-w-screen-lg md:max-w-screen-md lg:max-w-screen-xl px-4 py-16 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-2 shadow-lg bg-white rounded-md  w-full sm:w-full m-8">
